...
class MyStage(object):
    ...
    def __init__(self, os, serial_module, n_xips=1):
        ...
        #print("\n Command exit status/return code : ", p_status)
            #print("\n In case MyStage.connect() returns error: Access denied, 
            #check that the correct serialPort was used and launch 
            #MyStage.get_permission(serialPort) again.")
...
#        print('home set and saved to Corvus Memory')
    ...
    def go_home(self):
        self.write(b'0_0_0_m_')
        self.wait_for_movement()
        response = self.get_pos()
        print( 'new position:\n' + str(response))

    def get_pos(self):
        response = self.send(b'pos_')[:-2]
        pos = [float(val) for val in response.split()]
        return pos
    
    def set_velocity(self, velocity):
        self.send(b'%f_setvel_' %(velocity))
        print('Maximum Velocity set to %f.'%(velocity))
    
    def moving(self):
        moving = False
        for i in range(3):
            if self.send(b'%i_nst_'% (i+1))[:1] == str(0):
                pass
            else:
                moving = True
        return moving
    ...
    def send(self, command):
        ...
        self.write(command)
        response = self.read().decode('utf-8')
        return response
    ...
    def read(self):
        response = self.connection.readall()
        return response
...
class MyAxes(object):
    ...
    def __init__(self, stage, x=False, y=False, z=False):
    ...
    def move_relative(self, *xyz):
        ...
        self.send(b'%f_%f_%f_r_' % (moves[0], moves[1], moves[2]))  
        self.parent.wait_for_movement()
        response = self.parent.get_pos()
        print( 'new position:\n' + str(response))
        
    def move_global(self, *xyz):
        moves = self.parent.get_pos()
        i=0
        for arg in xyz:
            while True:
                if i>2:
        ...
        self.send(b'%f_%f_%f_m_' % (moves[0], moves[1], moves[2]))  
        self.parent.wait_for_movement()
        response = self.parent.get_pos()
        print( 'new position:\n' + str(response))
